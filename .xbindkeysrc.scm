(define (normal-bindings)
  ...
  (xbindkey '(Control Mod1 grave) "dunstctl history-pop")
  (xbindkey '(Control grave) "dunstctl close")
  (xbindkey '(Mod4 a) "rofi -show combi -modes combi -combi-modes drun,run -display-combi Launch -disable-history")
  (xbindkey '(Mod4 b) "blueman-manager & disown")
  (xbindkey '(Mod4 c) "qalculate-gtk & disown")
  (xbindkey '(Mod4 j) "$HOME/.local/lib/dotfiles/misc_scripts/word_count & disown")
  (xbindkey '(Mod4 p) "keepassxc & disown")
  (xbindkey '(Mod4 s) "flameshot gui & disown")
  (xbindkey '(Mod4 u) "gucharmap & disown")
  (xbindkey '(Mod4 v) "pavucontrol & disown")
  (xbindkey '(Mod4 w) "firefox & disown")
  (xbindkey '(Pause) "$HOME/.local/lib/dotfiles/misc_scripts/focus_stack 0.7")
  (xbindkey '(Shift Mod4 w) "firefox --private-window & disown")
  (xbindkey '(Mod1 Mod4 w) "chromium & disown")
  (xbindkey '(Shift Mod1 Mod4 w) "chromium --incognito & disown")
  ...
  ;; MS keys
  (xbindkey '(Control Shift Mod1 Mod4 m) "gtk-launch chrome-ocdlmjhbenodhlknglojajgokahchlkk-Default.desktop & disown")
  (xbindkey '(Control Shift Mod1 Mod4 n) "mate-terminal --name vim -e vim --disable-factory & disown")
  (xbindkey '(Control Shift Mod1 Mod4 o) "libreoffice & disown")
  (xbindkey '(Control Shift Mod1 Mod4 space) "xdotool click 1")
  (xbindkey '(Control Shift Mod1 Mod4 t) "gtk-launch chrome-cifhbcnohmdccbgoicgdjpfamggdegmo-Default.desktop & disown")
  )
...
(define web (launch-or-focus "--classname '^(Navigator|chromium)$'" "firefox & disown"))
(define email (launch-or-focus "--classname '^evolution$'" "evolution & disown"))
(define spotify (launch-or-focus "--classname '^spotify$'" "spotify & disown"))
(define office (launch-or-focus "--classname '^(libreoffice|lyx|code)$'" "$HOME/.local/lib/dotfiles/misc_scripts/launch_office"))
(define terminal (launch-or-focus "--classname '^mate-terminal$'" "mate-terminal & disown"))
...
(define (ergo-bindings)
  ...
  (xbindkey-function '(c) (lambda ()
							(if (eq? key-repeat 0)
							  (ergo-cmd "xdotool getactivewindow windowquit")
							  (ergo-cmd (string-append/shared "wmctrl -l | $HOME/.local/lib/dotfiles/misc_scripts/window_stack -vafter_active=1 -vprint_prefix='windowquit ' -vnum_windows=" (number->string key-repeat) " | xdotool -")))))
  ...
  (xbindkey-function '(f) (lambda () (if (not (eq? key-repeat 0))
									  ...
									  (if (eq? fold-depth 1)
										(ergo-cmd "wmctrl -l | $HOME/.local/lib/dotfiles/misc_scripts/window_stack -vcurr_workspace=1 -vafter_active=1 -vprint_prefix='windowactivate ' -vskip_first=1 | tac | xdotool -")
										(ergo-cmd (string-append/shared "wmctrl -l | $HOME/.local/lib/dotfiles/misc_scripts/window_stack -vcurr_workspace=1 -vafter_active=1 -vprint_prefix='windowactivate ' -vskip_first=1 -vnum_windows=" (number->string fold-depth) " | tac | xdotool -")))))
;  (xbindkey-function '(f) (lambda ()
;							(ergo-cmd (string-append/shared "wmctrl -l | $HOME/.local/lib/dotfiles/misc_scripts/window_stack -vprint_prefix='windowactivate ' -vskip_first=1 -vnum_windows=" (number->string key-repeat) " | xdotool -"))))
  (xbindkey-function '(g) (lambda () (ergo-cmd office)))
  (xbindkey-function '(grave) (lambda () (ergo-cmd "dunstctl action 0")))
  (xbindkey-function '(q) (lambda () (ergo-cmd "$HOME/.local/lib/dotfiles/misc_scripts/focus_stack")))
  (xbindkey-function '(space) (lambda () (ergo-cmd "$HOME/.local/lib/dotfiles/misc_scripts/focus_stack 0.7")))
  ...
  (xbindkey-function '(h) (lambda () (ergo-cmd "$HOME/.local/lib/dotfiles/misc_scripts/focus_direction left")))
  (xbindkey-function '(j) (lambda () (ergo-cmd "$HOME/.local/lib/dotfiles/misc_scripts/focus_direction down")))
  (xbindkey-function '(k) (lambda () (ergo-cmd "$HOME/.local/lib/dotfiles/misc_scripts/focus_direction up")))
  (xbindkey-function '(l) (lambda () (ergo-cmd "$HOME/.local/lib/dotfiles/misc_scripts/focus_direction right")))
